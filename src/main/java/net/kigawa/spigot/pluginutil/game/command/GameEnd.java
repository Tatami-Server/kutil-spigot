...
public class GameEnd extends GameCommandBase {
    public GameEnd(PluginBase kigawaPlugin, GameManagerBase gameManagerBase, CommandParent commandParent) {
    ...
    public String onThisCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == getWordNumber() + 1) {
            return getManager().end(strings[getWordNumber()]);
        }
        return null;
    }
