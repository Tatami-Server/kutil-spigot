...
public class Remove extends GameCommandBase {
    public Remove(PluginBase kigawaPlugin, GameManagerBase gameManagerBase, CommandParent commandParent) {
    ...
    public String onThisCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == getWordNumber() + 1) {
            return getManager().remove(strings[getWordNumber()]);
        }
        return null;
    }
